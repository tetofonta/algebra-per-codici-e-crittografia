...
\end{frame}
\note{
    \ref{snum:keylength} L'unica protezione contro attacchi bruteforce sta nel tempo di transazione di alcuni millisecondi ma nessuna protezione da attacchi su ciphertext.

    \ref{enum:rng-16-bit} Dalla analisi dei pacchetti è stato possibile notare che lo stato del RNG è di soli 16 bit. 
    questo comporta l'esistenza di soli 65536 possibili nonce e che la seconda parte di $n_t$ è funzione della prima. in particolare è stato possibile identificare che un nonce è valido se e solo se è rispettato \[n_k \oplus n_k+2 \oplus n_k+3 \oplus n_k+5 \oplus n_k+16 = 0\]
    ...
    \ref{enum:rng-reader-update} Questo implica che possono esistere numerosi lettori vulnerabili a attacchi sul parametro $n_r$, infatti è possibile notare che la sequenza dei nonce $n_r$ a partire dall'avvio del lettore non varia.
}

\begin{frame}
    \frametitle{Funzione di filtraggio dipoendente da bit in posizione dispari}
    La funzione di filtraggio è stata identificata ed è stato notato come solo i bit in posizone dispari sono input ad essa. \pause

    I bit dispari dello stato sono responsabili della codifica dei bit dispari del messaggio\pause
    
    I bit pari dello stato sono responsabili della codifica dei bit pari del messaggio\pause
\end{frame}
\note{
    ...
    bisogna trovare la sequenza $\bar{r} = r_0, r_1, r_2 ... r_{46+n}$ tale per cui la relazione 
    \begin{multline*}

...
\note{
    ...
    \begin{itemize}
        ...
        \item una volta ottenuto il nonce, il lettore computa i primi 32 bit del keystream
        \item il lettore genera \(n_r\) e lo invia al tag cifrandolo con \(ks_1\) \(n_r \oplus ks_1\) insieme invia l'iterazione successiva \(a_r = suc^2(n_t)\)
        \item il tag quindi, aggiorna il proprio cifrario con \(n_r\), quindi genera \(ks_2\) che permetterà di verificare la correttezza di \(suc^2(n_t)\)

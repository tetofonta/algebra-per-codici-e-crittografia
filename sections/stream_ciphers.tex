...
\note{
    ...
    In sè OTP è un cifrario perfetto, dato che non permette a un ascoltatore di inferire sulle probabilità che i bit del testo sorgente siano 1 o 0; ma ciò è valido solo se la chiave è completamente randomica e non venga mai riutilizzata.

    Gli stream cipher sono stati dunque ideati al fine di generare continuamente bit con i quali cifrare la chiave, facendo sì che il bit non dipenda dai precedenti e che esso habbia uguale probabilità di essere in uno stato o nell'altro.
...
\end{frame}
\note{
    In questo caso la fiunzione $g$ è \[g(b_0, b_1, ... b_{16}) = b_{16} \oplus b_{14} \oplus b_{13} \oplus b_{11}\]
